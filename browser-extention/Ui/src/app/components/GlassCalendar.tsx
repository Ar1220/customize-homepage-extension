...
export function GlassCalendar({ color = "#c58bf2", textColor = "#ffffff", transparentBg = false, blur = 24, strokeEnabled = false, strokeColor = "#c58bf2", bgOpacity = 30 }: GlassCalendarProps) {
  ...
  const renderHeader = () => {
    return (
      <div className="flex justify-between items-center mb-6">
        <h2 
          className="text-2xl font-playfair italic font-medium tracking-wide flex items-center gap-3"
          style={strokeEnabled ? { color: textColor, WebkitTextStroke: `1px ${strokeColor}` } : { color: 'white' }}
        >
  ...
  const renderDays = () => {
    ...
    for (let i = 0; i < 7; i++) {
      days.push(
        <div 
          key={i} 
          className="text-center text-[10px] uppercase tracking-[0.2em] font-space mb-2"
          style={strokeEnabled ? { color: textColor, WebkitTextStroke: `1px ${strokeColor}`, opacity: 0.8 } : { color: 'rgba(255,255,255,0.4)' }}
        >
      ...
      for (let i = 0; i < 7; i++) {
        ...
        days.push(
          <button
            ...
            className={`
              relative h-10 w-full flex flex-col items-center justify-center rounded-full transition-all
              font-space text-sm
              ${!inMonth ? "text-white/20 hover:text-white/40" : "text-white/80 hover:bg-white/10 hover:text-white"}
              ${today && !isSelected ? "font-bold text-white" : ""}
  ...
  return (
    <div
      ...
      className={`relative flex flex-col md:flex-row p-8 rounded-[3rem] overflow-hidden pointer-events-auto group min-w-[340px] md:min-w-[700px] ${transparentBg ? '' : 'shadow-2xl'}`}
      style={glassStyle}
    >
      ...
      <div className="flex-1 md:pr-8 md:border-r border-white/10 relative z-10">
        {renderHeader()}
        {renderDays()}
        {renderCells()}
      </div>
      ...
      <div className="flex-1 mt-8 md:mt-0 md:pl-8 flex flex-col relative z-10">
        <AnimatePresence mode="wait">
          {isEditing && selectedDate ? (
            ...
            >
              <h3 
                className="text-lg font-space font-medium mb-6 flex items-center gap-2"
                style={strokeEnabled ? { color: textColor, WebkitTextStroke: `1px ${strokeColor}` } : { color: 'white' }}
              >
                  ...
                  upcomingNotes.map((item, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-sm font-space line-clamp-2" style={{ color: textColor }}>
                          {item.note}
                        </p>
                      </div>
                    );
