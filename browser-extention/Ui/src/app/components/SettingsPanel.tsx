...
import { X, Image as ImageIcon, Link2, Video, Move, Upload, Palette, Volume2, VolumeX, Terminal, Trash2 } from "lucide-react";

import { Bookmark, SearchCommand, BackgroundFilters, FILTER_PRESETS, CropTarget, CroppingContext } from "../App";
...
interface SettingsPanelProps {
  ...
  onUpdateSearchCommands?: (commands: SearchCommand[]) => void;
  ...
  tabIcon?: string;
  onUpdateTabIcon?: (icon: string) => void;
  croppingImage?: CroppingContext | null;
  setCroppingImage?: (state: CroppingContext | null) => void;
  onResetAll?: () => void;
}
...
export function SettingsPanel({
  ...
  const handleAddCommand = () => {
    if (newCommandTrigger.trim() && newCommandName.trim() && newCommandUrl.trim()) {
      // Validate trigger doesn't have spaces or special chars except letters/numbers
      ...
      // Validation removed: URLs without {query} are treated as static navigational shortcuts
  ...
  const handleRemoveCommand = (id: string) => {
    onUpdateSearchCommands?.(searchCommands.filter(cmd => cmd.id !== id));
  };
  ...
  const handleBookmarkIconInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0] && setCroppingImage) {
      setCroppingImage({ url: URL.createObjectURL(e.target.files[0]), type: 'bookmark' });
    }
  };

  const handleSearchIconInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0] && setCroppingImage) {
      setCroppingImage({ url: URL.createObjectURL(e.target.files[0]), type: 'searchLogo' });
    }
  };
  ...
  const processFile = (file: File) => {
    ...
    if (isVideoFile) {
      ...
      reader.onloadend = () => {
        if (typeof reader.result === 'string') {
          onUpdateMedia(reader.result, true);
        }
      };
    ...
    const reader = new FileReader();
    reader.onloadend = () => {
      const rawImgUrl = reader.result as string;
      const img = new Image();
      img.onload = () => {
        ...
        onUpdateMedia(compressedBase64, false);
      };
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="absolute right-0 top-0 bottom-0 z-50 w-full max-w-sm border-l border-white/10 bg-black/90 backdrop-blur-xl p-6 overflow-y-auto no-scrollbar shadow-2xl flex flex-col custom-scrollbar"
          >
            ...
            <div className="space-y-8 flex-1 pb-8">
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-4">
                  <div>
                    ...
                    <input 
                      ...
                      style={{ borderBottomColor: currentTheme, borderBottomWidth: '1px' }}
                    />
                  </div>
                  <div>
                      ...
                      <div className="relative flex-1">
                        <input 
                          ...
                          onChange={(e) => {
                            ...
                            if (file && setCroppingImage) {
                              setCroppingImage({ url: URL.createObjectURL(file), type: 'tabIcon' });
                            }
                          }}
                        />
                        <button className="w-full px-3 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-sm transition-colors flex items-center justify-center gap-2 font-space text-white">
                  ...
                  <div className="flex items-center justify-between px-4 py-3 bg-white/5 border border-white/10 rounded-lg">
                    ...
                    <button className="relative inline-flex h-5 w-9 items-center rounded-full transition-colors shrink-0 bg-white">
                      <span className="inline-block h-3 w-3 transform rounded-full bg-black transition-transform translate-x-5"></span>
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4 bg-white/5 p-4 rounded-xl border border-white/5 mt-6">
                <div className="flex items-center gap-2">
                  <Terminal size={18} className="text-white/70" />
                  <h3 className="text-sm font-medium text-white font-space">Custom Search Commands</h3>
                </div>

                <p className="text-xs text-white/50 font-space leading-relaxed">
                  Add quick commands for your Spotlight Search. Type <span className="bg-white/10 px-1 py-0.5 rounded text-white/70">/yt query</span> to search YouTube, <span className="bg-white/10 px-1 py-0.5 rounded text-white/70">/g query</span> for Google, etc.
                </p>
                  ...
                  {searchCommands.map((cmd) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-space text-white truncate">
                          <span className="text-white/50">/</span>{cmd.trigger} - {cmd.name}
                        </p>
                        <p className="text-xs text-white/30 truncate">{cmd.url}</p>
                      </div>
                      <button
                ...
                <div className="flex flex-col gap-3 pt-3 border-t border-white/10">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative w-10 h-10 flex-shrink-0">
                      {newCommandIcon ? (
                        <img src={newCommandIcon} alt="Command icon" className="w-full h-full rounded object-cover" />
                      ) : (
                        <div className="w-full h-full rounded bg-white/5 flex items-center justify-center border border-white/10">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const file = e.target.files?.[0];
                          if (file && setCroppingImage) {
                            setCroppingImage({ url: URL.createObjectURL(file), type: 'command' });
                          }
                        }}
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <p className="text-xs text-white/40 font-space">
                      Use <span className="text-white/60">{'{query}'}</span> where search term goes. Example: https://youtube.com/results?search_query={'{query}'}
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    className="rounded-lg bg-white/10 border border-white/20 px-4 py-2 text-xs font-semibold text-white transition-all hover:bg-white/20 disabled:opacity-30 disabled:cursor-not-allowed font-space uppercase tracking-widest"
                  >
                    Add Command
                  </button>
                </div>
              ...
              <div className="space-y-4 pt-6 border-t border-white/10">
                <p className="text-sm text-white/50 leading-relaxed font-space">
                  Messed up? Restore the default look and text.
                </p>
                <button
                  onClick={handleReset}
                  className="w-full rounded-lg border border-white/10 bg-transparent px-4 py-3 text-sm font-medium text-white transition-all hover:bg-white/5 hover:border-white/20 font-space text-center"
                >
                  Reset to Default
                </button>
              </div>
            ...
            <div className="mt-6 pt-4 border-t border-white/10 flex flex-col gap-2">
              <a href="https://github.com/Ar1220/customize-homepage-extension" target="_blank" rel="noopener noreferrer" className="group relative flex items-center justify-center gap-2 w-full h-12 border border-white/10 bg-neutral-900/60 text-gray-50 text-xs font-bold uppercase tracking-widest rounded-lg overflow-hidden transition-all duration-500 hover:border-red-500/50 hover:text-red-200 before:absolute before:w-12 before:h-12 before:content-[''] before:right-1 before:top-1 before:z-0 before:bg-red-800 before:rounded-full before:blur-lg before:duration-500 after:absolute after:z-0 after:w-20 after:h-20 after:content-[''] after:bg-red-500 after:right-8 after:top-3 after:rounded-full after:blur-lg after:duration-500 hover:before:right-12 hover:before:-bottom-8 hover:after:-right-8 hover:before:[box-shadow:_20px_20px_20px_30px_#7f1d1d] group-hover:before:duration-500 group-hover:after:duration-500">
                <svg height="14" width="14" viewBox="0 0 16 16" fill="currentColor" className="relative z-10 text-white group-hover:text-red-300 transition-colors duration-300">
                  <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"></path>
                </svg>
                <span className="relative z-10 text-white group-hover:text-red-200 transition-colors duration-300">Support on GitHub</span>
              </a>
            </div>
          </motion.div>
        </>
      ...
      {croppingImage && setCroppingImage && (
        <ImageCropperModal
          ...
          targetType={croppingImage.type}
          onCrop={(croppedResult) => {
            switch (croppingImage.type) {
              case 'wallpaper':
                onUpdateMedia(croppedResult, false);
                break;
              case 'command':
                setNewCommandIcon(croppedResult);
                break;
              case 'bookmark':
                setBookmarkIconUrl(croppedResult);
                break;
              case 'tabIcon':
                if (onUpdateTabIcon) onUpdateTabIcon(croppedResult);
                break;
              case 'searchLogo':
                if (onUpdateSearchIcon) onUpdateSearchIcon(croppedResult);
                break;
            }
